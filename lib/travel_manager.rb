...
module TravelManager
  ...
  def self.itinerary(input_file, based)
    ...
    return 'ERROR: there was an error parsing the reservations, please review the input file' if unsorted_segments.empty?

    sorted_trips = TripBuilder.build(unsorted_segments, based)

    return 'ERROR: there was an error building the trips, please review the input file' if sorted_trips.nil? || sorted_trips.empty?

    sorted_trip_texts = TextFormatter.trips_to_text(sorted_trips)

    return 'ERROR: there was an error formatting the trips, please review the input file' if sorted_trip_texts.nil? || sorted_trip_texts.empty?
  ...
  def self.build_itinerary(sorted_trips)
    itinerary = ''
  ...
  private_class_method :build_itinerary, :validate_based!
end
